...
<script>
  ...
  import { firestoreManager } from '@/databases'
  import compareCollections from '@/Modules/Requirements/Templates/compareCollections'
  ...
  export default {
    ...
    computed: {
      currentDatabaseCollections () {
        return firestoreManager.entities
      },
      collectionsArray () {
    ...
    methods: {
      ...
      saveRequirement () {
        this.$emit('save', this.newCollectionRequirement)
      },
      addRequirement () {
        this.$emit('add', this.newCollectionRequirement)
      }
    }
  }
</script>
